...
window.plus_main = () => {
    ...
    const initialize_modules = () => {
        try {
            // Verificar que todos los módulos requeridos estén disponibles
            for (const [key, moduleName] of Object.entries(WA_MODULES)) {
                const module = require(moduleName);
                if (!module) {
                    console.warn(`Module ${moduleName} not available yet`);
                    return false;
                }
                MODULES[key] = module;
            }
    ...
    // Función para intentar inicializar los módulos con reintentos
    const initializeWithRetry = (maxRetries = 5, delay = 2000) => {
        let retries = 0;
        ...
        const tryInitialize = () => {
            ...
            if (retries < maxRetries) {
                console.log(`Retrying module initialization (${retries}/${maxRetries})...`);
                setTimeout(tryInitialize, delay);
            } else {
                console.error('Failed to initialize modules after maximum retries');
            }
        };
    ...
    class HookRendered extends Hook {
        ...
        register() {
            ...
            if (!MODULES.MESSAGES_RENDERER || !MODULES.MESSAGES_RENDERER.Meta) {
                console.warn('MESSAGES_RENDERER module not available yet, retrying in 1 second...');
        ...
        unregister() {
            ...
            super.unregister();
            if (MODULES.MESSAGES_RENDERER && this.original_function) {
        ...
        static device_handler(message) {
            ...
            const div_svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
